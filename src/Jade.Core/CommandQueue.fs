...
/// Pattern: urn:schema:{app}:command:{aggregate-type}:{action}:{version}
/// Example: "urn:schema:myapp:command:orders:create:1" -> "orders"
let extractAggregateType (dataschema: string) : Result<string, string> =
    match dataschema.Split(':') with
    | [| "urn"; "schema"; _; "command"; aggregateType; _; _ |] -> Ok aggregateType
    | _ -> Error $"Invalid dataschema URN format: {dataschema}. Expected pattern: urn:schema:{{app}}:command:{{aggregate-type}}:{{action}}:{{version}}"
        ...
        | Some data ->
            ...
            | Ok command ->
                ...
                | None ->
                    ...
                    return Error $"No handler registered for command type {command.GetType().Name}"
                | Some handler ->
                    ...
                    match result with
                    | Ok () -> logger.LogInformation("Successfully processed CloudEvent {Id}", cloudEvent.Id)
                    | Error err -> logger.LogError("Failed to process CloudEvent {Id}: {Error}", cloudEvent.Id, err)
                    return result
}
